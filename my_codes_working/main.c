...
//    char op;
    ...
    // reading operator
    if (scanf("%c", &arr_operators[0]) == 1) // check symbol
    {
//        arr_operators[0] = op;
        ret = EXIT_SUCCESS;
    } else {
    ...
    printf("operator in array: %c\n", arr_operators[0]);
//    printf("operator: %c\n", op);
    // only printing first matrix in our matrix array
    int count = 0;
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            printf("%d ", *(arr_matrixs[0] + i + j + count)); /* printing pointer to first matrix in array*/
        }
        count += 3;
        printf("\n");
    }
